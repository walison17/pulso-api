...
class PulsoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ('created_by', 'ends_at', 'created_at')

    def validate_radius(self, value):
        if not value >= settings.PULSO_MIN_RADIUS or not value <= settings.PULSO_MAX_RADIUS:
            raise serializers.ValidationError(
                f'Distãncia deve ser entre {settings.PULSO_MIN_RADIUS} e \
                    {settings.PULSO_MAX_RADIUS} metros.'
            )

        return value

    def create(self, validated_data):
        if Pulso.objects.happening().created_by(validated_data['created_by']).exists():
            raise serializers.ValidationError(
