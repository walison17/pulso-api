...
class TestPulsoSerializer(APITestCase):
    ...
    def test_contains_expected_fields(self):
        self.assertEqual(
            self.serializer.data['description'], self.pulso_payload['description']
        )

    def test_contains_expected_pulso_creator(self):
        self.assertEqual(
            self.serializer.data['created_by'], self.pulso_payload['created_by']
        )

    def test_contains_expected_comments_count(self):
        mommy.make('comments.Comment', pulso=self.pulso, _quantity=10)
        self.assertEqual(self.serializer.data['comments_count'], 10)
    ...
    def test_raises_exception_when_user_cannot_create_new_pulso(self):
        ...
        with self.assertRaises(serializers.ValidationError):
            serializer = PulsoSerializer(data=self.pulso_payload)
            serializer.is_valid(raise_exception=True)
...
class TestLocationSerializer(APISimpleTestCase):
    ...
    def setUp(self):
        self.location = {'lat': -8.2777392, 'long': -35.9716179}
        self.serializer = LocationSerializer()

    def test_internal_value(self):
        self.assertIsInstance(self.serializer.to_internal_value(self.location), Point)
